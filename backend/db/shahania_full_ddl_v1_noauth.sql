------------------------------
-- 0) Extensions & Housekeeping
------------------------------
CREATE EXTENSION IF NOT EXISTS pgcrypto;   -- for gen_random_uuid()
CREATE EXTENSION IF NOT EXISTS "uuid-ossp"; -- optional

------------------------------
-- 0.1) Common trigger: auto-update updated_at
------------------------------
...
------------------------------
-- 1) Identity & Auth (Django-compatible + scoped RBAC)
--    Note: If Django will manage auth_*, keep IF NOT EXISTS to avoid clashes.
------------------------------
...
-- Core RBAC for platform (independent from Django's permissions)
...
-- Optional contextual scoping for RBAC (school/year/class)
...
CREATE TRIGGER tr_identity_role_scope_u
BEFORE UPDATE ON identity_role_scope
FOR EACH ROW EXECUTE FUNCTION set_updated_at();

------------------------------
-- 2) Core Academics (school / year / term / subject / room / class)
------------------------------
...
-- Reference tables (grade/subject types)
...
CREATE TABLE IF NOT EXISTS people_student (
  ...
  nationality_code VARCHAR(5),      -- FK -> refdata_lookup_nationality
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  deleted_at TIMESTAMPTZ,
  row_version INT NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS people_guardian (
  ...
  phone         VARCHAR(32) NOT NULL, -- [SENSITIVE: CONTACT]
  email         VARCHAR(254) NOT NULL, -- [SENSITIVE: CONTACT]
  relation_to_student VARCHAR(30),     -- father/mother/…
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  deleted_at TIMESTAMPTZ,
  row_version INT NOT NULL DEFAULT 1
...
------------------------------
-- 5) Assessment (exam / session / result / appeal)
------------------------------
...
CREATE TABLE IF NOT EXISTS behavior_behaviorincident (
  ...
  category VARCHAR(80) NOT NULL,     -- refdata category optional
...
------------------------------
-- 11) Quality & Accreditation (QNSA) + Operational Plan & Committees
------------------------------
-- Committees (can serve as Quality / Self-Review / Operational via type)
...
-- Executors list (department coordinators etc.)
...
-- Operational Plan Items (big Excel)
...
-- Many executors per plan item (optional bridge)
...
------------------------------
-- 13) Communications (multichannel)
------------------------------
...
------------------------------
-- 14) Audit & Outbox (observability & future integrations)
------------------------------
...
------------------------------
-- 15) Refdata (lookups with i18n potential)
------------------------------
...
------------------------------
-- Index notes: add more if specific reports show slowness.
------------------------------